...
namespace Интерпретатор_машины_Тьюринга
{
    ...
    public partial class Form1
    {
        ...
        private void OnSessionManagerAssignmentChanged(int assignmentId, string reason, string message)
        {
            ...
            if (reason == "MTUpdated")
            {
                // По UX-требованию: в окне «Выполнение задания» никакие элементы об обновлении МТ
                // не показываются — ни плашки, ни кнопки «Начать с новой МТ». Студент увидит
                // информацию об изменении МТ в окне «Информация о задании» (надпись над кнопками).
                // Здесь показываем лишь неблокирующее уведомление и не блокируем работу.
                ShowWarningDialog("Преподаватель обновил конфигурацию МТ задания.\n\nВаше текущее решение основано на старой версии. При необходимости сохраните его в файл через «Файл → Экспорт программы».");
                return;
            }
